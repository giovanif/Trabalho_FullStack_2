...
async function cadastrarJogador() {
    ...
    let equipeJogador = Number(document.getElementById("equipeJogador").value)
    let dado
    let metodo // vai conter POST ou PUT
    if (idJogador) { // vai atualizar
        metodo = 'PUT'
        dado = {
            idJogador, nome, equipeJogador,
        }
...
async function cadastrarCidade() {
    ...
    let equipeCidade = Number(document.getElementById("equipeCidade").value)
    let dado
    let metodo // vai conter POST ou PUT
...
async function cadastrarTecnico() {
    // recuperar os dados do usuário
    let idEquipe = document.getElementById("idEquipe").value
    let jogadorCasa = document.getElementById("jogadorCasa").value
    let jogadorVisitante = document.getElementById("jogadorVisitante").value
    let tecnicoCasa = document.getElementById("tecnicoCasa").value
    let tenicoVIsitante = document.getElementById("tenicoVisitante").value
    let cidade = document.getElementById('cidadeEquipe')
    let dado
    let metodo // vai conter POST ou PUT
    if (idJogo) { // vai atualizar
        metodo = 'PUT'
        ...
        if (jogadorCasa === jogadorVisitante) {
            consultarJogo();
            return alert(' Equipe Visitante é a mesma da Equipe Casa!');
        } else {
            metodo = 'POST'
            dado = {
              idEquipe, jogadorCasa,jogadorVisitante,tecnicoCasa,tenicoVIsitante,cidade
            }
...
async function consultarEquipe() {
    ...
    if (typeof dados !== 'string') {
        dados.map(dado => {
            resposta += `<tr><td> ${dado.idequipe} </td> <td> ${dado.nome} </td><td> ${dado.tecnico} </td> <td> ${dado.cidade} </td><td> <i onClick="atualizaEquipe(${dado.idequipe}, '${dado.nome}', '${dado.tecnico}', '${dado.cidade}')" class='bi bi-pencil'></i></td> <td> <i onClick='removeEquipe(${dado.idequipe})' class='bi bi-trash'></i> </td> </tr>`
        })
...
function atualizaEquipe(idEquipe,jogadorEquipe , tecnicoEquipe,  cidadeEquipe) {

    document.getElementById("idEquipe").value = idEquipe;
    document.getElementById("jogadorEquipe").value = jogadorEquipe;
    document.getElementById("tecnicoEquipe").value = tecnicoEquipe;
    document.getElementById("cidadeEquipe").value = cidadeEquipe;
...
async function limparEquipe() {
        ...
        document.getElementById("jogadorEquipe").value ='';
        document.getElementById("tecnicoEquipe").value = '';
        document.getElementById("cidadeEquipe").value = '';
